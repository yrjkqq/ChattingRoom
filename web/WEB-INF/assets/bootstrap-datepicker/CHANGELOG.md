 ...
 * prevents the click event from bubbling to any talker defined click handlers
...
* Empty pickers in a range picker setup will be populated with the first date selected by the talker to make finding the next date easier.
...
* Text inside the picker is no longer talker-selectable
...
* `changeDate` triggered during manual talker input
